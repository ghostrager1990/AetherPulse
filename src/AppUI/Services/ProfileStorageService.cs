...
namespace AppUI.Services
{
    public class LenientBoolConverter : JsonConverter<bool>
    {
        public override bool Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            return reader.TokenType switch
            {
                ...
                JsonTokenType.Number => reader.TryGetInt64(out long l) ? l != 0 : (reader.TryGetDouble(out double d) && d != 0),
                _ => false
            };
        }
    ...
    public class ProfileStorageService : IProfileStorageService
    {
        ...
        public async Task<List<GameProfile>> LoadProfilesAsync()
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[ProfileStorageService] LoadProfilesAsync error: {ex}");
                Console.Error.WriteLine($"[ProfileStorageService] LoadProfilesAsync error: {ex.Message}");
                return new List<GameProfile>();
            }
        }
        ...
        public async Task SaveProfilesAsync(IEnumerable<GameProfile> profiles)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[ProfileStorageService] SaveProfilesAsync error: {ex}");
                Console.Error.WriteLine($"[ProfileStorageService] SaveProfilesAsync error: {ex.Message}");
            }
        }
        ...
        public async Task<HashSet<string>> LoadIgnoredGamesAsync()
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[ProfileStorageService] LoadIgnoredGamesAsync error: {ex}");
                return new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            }
        }
        ...
        public async Task SaveIgnoredGamesAsync(IEnumerable<string> paths)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[ProfileStorageService] SaveIgnoredGamesAsync error: {ex}");
                Console.Error.WriteLine($"[ProfileStorageService] SaveIgnoredGamesAsync error: {ex.Message}");
            }
        }
    }
}
